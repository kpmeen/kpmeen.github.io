      ...
      <.svg(Styles.svg,
        <.defs(
          <.maskTag(^.id := "mask", ^.x := "0", ^.y := "0", ^.width := "100%", ^.height := "100%",
            <.rect(Styles.alpha, ^.id := "alpha", ^.x := "0", ^.y := "0", ^.width := "100%", ^.height := "100%"),
            <.text(Styles.title, ^.id := "title", ^.x := "50%", ^.y := "0", ^.dy := "1.58em", props.siteTitle),
            <.text(Styles.subTitle, ^.id := "subtitle", ^.x := "50%", ^.y := "0", ^.dy := "9.8em", subTitle.mkString(" "))
          )
        ),
        <.rect(Styles.base, ^.id := "base", ^.x := "0", ^.y := "0", ^.width := "100%", ^.height := "100%")
      )
    }
  }

  val component = ScalaComponent.builder[Config]("HeaderSVG")
    .stateless
